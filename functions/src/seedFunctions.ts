...
export const seedMatchTest = onRequest(async (req, res) => {
  ...
  type SeedP = { id: string; role: 'chris' | 'kelly' }
...
export const seedGroupForTest = onRequest(async (req, res) => {
  ...
  const body = (req.body?.data ?? req.body) as {
    ...
    group_id?: unknown
    lead_id?: unknown
    chris_participants?: unknown
    kelly_participants?: unknown
  }
  ...
  if (!Array.isArray(body.chris_participants) || !Array.isArray(body.kelly_participants)) {
    res.status(400).json({ error: 'chris_participants and kelly_participants arrays required' }); return
  }

  const gameInstanceId = body.game_instance_id
  const groupId = body.group_id
  const leadId = body.lead_id
  const chrisPids = body.chris_participants as string[]
  const kellyPids = body.kelly_participants as string[]
