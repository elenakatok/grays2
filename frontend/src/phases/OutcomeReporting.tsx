...
function defaultFormValues(): FormValues {
  ...
  for (const field of graysSchema) {
...
export function parseForm(values: FormValues, schema: OutcomeSchema = graysSchema): ParseOk | ParseErr {
...
export default function OutcomeReporting({
  ...
  const roleKey   = groupData.chris_participants.includes(participantId) ? 'chris' : 'kelly'
  const roleLabel = labelFor(graysConfig, roleKey)
